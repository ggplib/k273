...
greenlet_t *greenlet_new_debug(greenlet_start_func_t start_func,
                               greenlet_t *parent, long stacksize);
...
greenlet_t* createGreenlet(const Callable& f, greenlet_t* parent=nullptr) {
    greenlet_t* g = greenlet_new(_greenlet_hidden::WrapSimpleLambda::call,
                                 parent, 0);

    if (g == nullptr) {
        K273::l_critical("Failed to create grenlet");
        ASSERT_MSG(false, "Failed to create grenlet.");
    }
